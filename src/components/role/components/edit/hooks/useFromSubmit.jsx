...
export default function useFormSubmit() {
  ...
  const editRoleMutate = useMutation({
    mutationFn: (data) => {
      ...
      return putRequest(`${ROLE.EDIT}${id}`, reqData);
    },
  });
  ...
  const handleFormSubmit = async (values) => {
    return editRoleMutate.mutateAsync(values, {
      onSuccess: (response) => {
        toast.success(response?.message || 'Role Updated'); // Show success toast notification
        navigate(ROUTES_ENUMS.USER_MANAGEMENT.ROLE_MANAGEMENT.INDEX); // Navigate to the role management page
      },
