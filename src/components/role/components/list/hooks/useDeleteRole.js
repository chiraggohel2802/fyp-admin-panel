 ...
 *
 * @param {Function} refetchRoleList - Function to refetch the role list after deletion.
...
export default function useDeleteRole(refetchRoleList) {
  ...
  const deleteRoleMutation = useMutation({
    mutationFn: (id) => deleteRequest(`${ROLE.DELETE}${id}`),
    onSuccess: (response) => {
      refetchRoleList(); //role list refetching data
      toast.success(response?.message || 'Role Deleted'); // Show success toast notification
    },
  ...
  const handleDeleteRole = async (id) => {
    Confirmation('Are you sure you want to delete this role?').then(
      async (deleteConfirm) => {
